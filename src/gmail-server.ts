...
class GmailMCPServer {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          ...
          case 'send_email':
            return await this.sendEmail(args as unknown as SendEmailRequest);
          case 'search_emails':
          ...
          case 'create_draft':
            return await this.createDraft(args as unknown as SendEmailRequest);
          case 'get_auth_url':
  ...
  private async readEmails(args: any) {
  ...
  private async sendEmail(args: SendEmailRequest) {
  ...
  private async getLabels() {
    ...
    const labelSummary = `# Gmail Labels\n\n**Total Labels**: ${labels.length}\n\n` +
      labels.map((label: any) => 
        `- **${label.name}** (${label.id}) - Type: ${label.type || 'user'}, Messages: ${label.messagesTotal || 0}`
      ).join('\n');
  ...
  private async getEmailThread(args: any) {
  ...
  private async addLabels(args: any) {
    const messageIds: string[] = args.message_ids;
    const labelIds: string[] = args.label_ids;
  ...
  private async removeLabels(args: any) {
    const messageIds: string[] = args.message_ids;
    const labelIds: string[] = args.label_ids;
  ...
  private async deleteLabel(args: any) {
    const labelId: string = args.label_id;
  ...
  private async updateLabel(args: any) {
    const labelId: string = args.label_id;
    ...
    if (labelListVisibility) requestBody.labelListVisibility = visibilityMap[labelListVisibility] || labelListVisibility;
  ...
  private async createDraft(args: SendEmailRequest) {
    const { to, subject, body, cc, bcc, html } = args;
  ...
  private async parseEmailMessage(message: any, includeBody: boolean): Promise<EmailMessage> {
  ...
  private extractEmailBody(payload: any): string {
    if (!payload) return 'Body content not available';
  ...
  private buildEmailSummary(emails: EmailMessage[], query: string, includeBody: boolean): string {
